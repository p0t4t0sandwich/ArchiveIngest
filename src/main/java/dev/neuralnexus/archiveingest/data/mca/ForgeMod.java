...
public record ForgeMod(
        ...
        @NonNull String forgeVersionRange
) implements Mod {
    public ForgeMod withLoaderSupport(List<LoaderSupport> loaderSupport) {
        return new ForgeMod(
                id, fileName, size, md5, sha1, sha256, sha512,
                related, links, info,
                modId, name, version, description, license,
                authors, contributors, loaderSupport,
                dependencies, platformRefs, side,
                forgeVersionRange
        );
    }
    ...
    public static @NonNull ForgeMod ingest(Path jarPath) throws IOException {
        ...
        if (forgeVersionRange == null) throw new IOException("Missing required field: loaderVersion");
        ...
        if (displayURL != null) links.add(new Link("home_page", displayURL));
